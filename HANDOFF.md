...
- **真机 e2e 全链路打通**（iPhone 14 / iOS 26.6.2 + Mac）
  ...
  - 键盘 / 鼠标注入：`RemoteCrab-e2e-OK` 真的打进 TextEdit 光标处（Accessibility 已授权）
  - **关键澄清**：之前"Mac 测试面板黑屏"不是解码 bug，是**后置摄像头被挡住**（手机扣在桌上）。
    诊断方法：`H264Decoder` 里有 env-gated 亮度探针（`REMOTECRAB_DEBUG_FRAME_PROBE=1`）。
- **多语言**：RemoteCrabCore 统一 String Catalog（`Sources/RemoteCrabCore/Resources/Localizable.xcstrings`，en + zh-Hans），
...
- 新增 `RemoteCrabMicDriver/`：`SharedRing.{h,c}`（C + stdatomic 无锁 SPSC 环，48k mono Int16，2s）、
  `RemoteCrabMicrophone.c`（CoreAudio HAL `AudioServerPlugIn`：驱动/设备/流三层，单**输入**设备
  48k/1ch/Float32，`DoIOOperation(ReadInput)` 从环读 Int16→Float32，render 路径零分配）、
  `MicRingBridge.{h,c}`（给 Swift 的不透明 C 桥）。
- `project-mac.yml` 新增 `RemoteCrabMicrophone`（`type: bundle`，`WRAPPER_EXTENSION: driver`，
  `CFPlugInFactories/Types` 指向 `RemoteCrabMicrophone_Create`），随 app 构建但不 embed。
- Mac app：`RemoteCrabReceiver/MicRingWriter.swift` + 桥接头 + `SharedRing.c/MicRingBridge.c`；
...
- **未验证**：需要 `sudo` 安装 + 在 App 里选「RemoteCrab Microphone」；
...
- 菜单栏图标从 SF Symbol 手机 → **monitor-buddy logo 模板图**
  （`RemoteCrabReceiver/Assets.xcassets/MenuBarIcon.imageset`，母版 `assets/menu-bar-icon.svg`，
...
- `./scripts/e2e-device.sh`：真机无头 e2e，构建+部署+跑全部 `REMOTECRAB_E2E_*` 标志，断言 receiver 日志 marker。
  **10/10 通过**：握手 / 视频 / 音频 / 触控 / 按键 / 文件传输 / 剪贴板 / 应用切换 / 录制。
- `./scripts/test.sh`：74 单测 + 双端 build。
- UI 审查：模拟器 `REMOTECRAB_E2E_SURFACE=trackpad|keyboard|camera` 预设界面后 `simctl io screenshot`。
...
- e2e：`REMOTECRAB_E2E_SURFACE=trackpad|keyboard|camera` 预设界面，便于模拟器截图审查。
...
- 单测覆盖 TextTransform + wire；**真机端到端未稳定验证**（手机锁屏/多实例陈旧 owner 干扰），
  建议在 TextEdit 里选中文字手动试一次。e2e：`REMOTECRAB_E2E_TEXT_COMMAND=uppercase`（+10s）。
...
- e2e flag `REMOTECRAB_E2E_CLIPBOARD=1`。
...
- **发送到 Mac**：iPhone 顶栏上传图标 → 照片/视频（PhotosPicker）或文件（fileImporter）
  ...
  → Mac 写入 `~/Downloads/RemoteCrab/` 并**在 Finder 中显示**；`fileAck 0x12` 回进度。
- **录制**：Mac 菜单栏 Record（⌘R）→ `~/Movies/RemoteCrab/recording-<stamp>.mov`（H.264，5s≈3-5MB）
...
- 真机验证：`file saved /Users/edwinhao/Downloads/RemoteCrab/…`；`.mov` 1920×1080 / 5.07s；
  `.wav` 473KB（开麦）。e2e flag：`REMOTECRAB_E2E_SEND_FILE=1`（iOS）、`REMOTECRAB_E2E_RECORD=1`（Mac）。
...
- e2e flag `REMOTECRAB_E2E_SWITCH=<bundleid>`。
...
- 多 Mac 配对**已真机验证**：
  ...
  - 第二台 Mac（`open -n -a … --args -remotecrab.mac.id second-mac-test`）→ `sessionReply: busy owner=MacBook Pro de Edwin`，
    约每 32s 一次慢重试（30s slow retry，**无 3s 重连风暴**），owner 不受影响（持续 4140+ 帧）

## 待做

- [ ] 相机扩展用户开关（系统设置 → 登录项与扩展 → 相机扩展 → 打开 RemoteCrab）
...
xcrun devicectl device process launch --device 866A1921-B588-59D5-A1B7-B266103B2E49 \
  --terminate-existing \
  --environment-variables '{"REMOTECRAB_AUTO_START":"1","REMOTECRAB_AUTOSTREAM":"1","REMOTECRAB_E2E_MIC":"1","REMOTECRAB_E2E_AUTOPAIR":"1"}' \
  com.ibridge.iBridgeCapture

# Mac 日志
log stream --predicate 'subsystem == "com.remotecrab"' --info --style compact

# 门禁
./scripts/test.sh              # 62 测试 + 双端 build
```

env flags：`REMOTECRAB_AUTO_START`（跳 onboarding）、`REMOTECRAB_AUTOSTREAM`（自动推流）、
`REMOTECRAB_E2E_MIC`、`REMOTECRAB_E2E_INPUT`（脚本化触控+打字）、`REMOTECRAB_E2E_VOICE`、
`REMOTECRAB_E2E_AUTOPAIR`（自动批准配对）。

## 环境注意

- 真机 build：`DEVELOPMENT_TEAM=5XNDF727Y6`（不是 project-*.yml 里的 DDG3CJL762）。
- Mac 端重签/换路径会让 Accessibility 失效；在**系统设置 → 辅助功能**里重新打开
  `/Applications/RemoteCrabReceiver.app`（本次重部署后授权保留，验证过）。
