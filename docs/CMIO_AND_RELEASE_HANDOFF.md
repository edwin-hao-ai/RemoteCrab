...
**目标**：让 “RemoteCrab Camera” 出现在 Photo Booth / Zoom / 飞书 的摄像头列表里。

**现状（已诊断）**
- `systemextensionsctl list` → `com.remotecrab.RemoteCrabReceiver.Camera (0.2/1)  [activated enabled]`。
...
**关键文件**
- 扩展：`RemoteCrabCameraExtension/Sources/RemoteCrabCameraExtension/`
...
- 工程：`project-mac.yml` 的 `RemoteCrabCameraExtension` target（`type: system-extension`,
  `CMIOExtension.CMIOExtensionMachServiceName`, `CFBundlePackageType: SYSX`）
- Host：`RemoteCrabReceiver/CameraExtensionBridge.swift`、`SystemExtensionManager.swift`
- 扩展嵌在 `/Applications/RemoteCrab.app/Contents/Library/SystemExtensions/…Camera.systemextension`
...
**命名**：扩展显示名已改为 **RemoteCrab Camera**（`CameraExtensionProvider/Device.swift`），
重新激活后才生效；旧激活实例仍显示 “RemoteCrab Camera”。

## B. 发布 / App Store Connect 资产

- **App**：name `RemoteCrab: Phone as Cam & Pad`，**appId `6811599153`**，
  bundleId `com.ibridge.iBridgeCapture`，SKU `remotecrab`，team `5XNDF727Y6`。
...
- **ASC API 凭证**：`~/.config/mddock/ios-release.env`
  ...
  已软链到仓库脚本读取的 `~/.config/remotecrab/ios-release.env`。
...
- **构建/上传**（详见 `docs/RELEASE.md`）：
  ...
  xcrun altool --upload-app -f build/asc/RemoteCrabCapture.ipa -t ios \
...
- 桌面 App 文件已改名 `/Applications/RemoteCrab.app`（带 AppIcon）；麦克风驱动 pkg = `dist/RemoteCrabMicrophone.pkg`。
