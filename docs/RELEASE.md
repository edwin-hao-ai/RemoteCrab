...
xcodebuild -exportArchive -archivePath build/iBridgeCapture.xcarchive \
  ...
  -allowProvisioningUpdates                                   # ✅ 导出 build/export/iBridgeCapture.ipa
```
...
3. 生成 App Store Connect API Key，写 `~/.config/ibridge/ios-release.env`（0600）：
   ```
   APPLE_API_KEY=XXXXXXXXXX
   APPLE_API_ISSUER=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
   APPLE_API_KEY_PATH=$HOME/.config/ibridge/AuthKey_XXXXXXXXXX.p8
...
xcrun notarytool submit iBridgeReceiver.dmg --keychain-profile <profile> --wait
xcrun stapler staple iBridgeReceiver.dmg
```
- 虚拟麦克风 pkg：`scripts/build-mic-driver-pkg.sh` → `dist/iBridgeMicrophone.pkg`；
  用 **Developer ID Installer** 证书 `productsign` + 公证，再嵌进 App；
  设置页「麦克风驱动」一键打开它。

## C. 本机验证
```sh
xcrun devicectl device install app --device <udid> build/export/iBridgeCapture.ipa   # 或 .app
```
