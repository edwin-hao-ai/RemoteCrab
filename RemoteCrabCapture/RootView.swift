...
struct RootView: View {
    ...
    @AppStorage("remotecrab.didOnboard") private var didOnboard: Bool = false
    /// Settings → "Replay Onboarding" flips this; RootView consumes it
    /// and drops back into the onboarding stage.
    @AppStorage("remotecrab.replayOnboarding") private var replayOnboarding: Bool = false
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Auto-start: when REMOTECRAB_AUTO_START=1 is set in the env
            // (e.g. via xcrun simctl launch --setenv or Xcode scheme),
            // skip onboarding + permissions and go straight to main.
            if didOnboard ||
                ProcessInfo.processInfo.environment["REMOTECRAB_AUTO_START"] == "1" {
                stage = .main
            } else {
                stage = .onboarding
            }
