#!/usr/bin/env bash
#
# Simulator-based e2e test for RemoteCrab.
#
# Boots iPhone simulator (or uses already-booted one), installs
# RemoteCrabCapture, launches it, and captures screenshots to prove the
...
APP_BUNDLE=$(find ~/Library/Developer/Xcode/DerivedData -name "RemoteCrabCapture.app" -path "*Debug-iphonesimulator*" 2>/dev/null | head -1)

mkdir -p "$OUTPUT_DIR"

echo "═══════════════════════════════════════════════════════"
echo "  RemoteCrab simulator e2e demo"
echo "═══════════════════════════════════════════════════════"
echo ""
...
if [[ -z "$APP_BUNDLE" ]]; then
    echo "❌ RemoteCrabCapture.app not built. Run:"
    echo "   xcodebuild -project RemoteCrabCapture.xcodeproj -scheme RemoteCrabCapture \\"
...
# Install the app
echo ""
echo "→ Installing RemoteCrabCapture..."
...
# Launch
echo ""
echo "→ Launching RemoteCrabCapture..."
...
To complete the full e2e (real iPhone → real Mac streaming):
  1. Open RemoteCrabReceiver.xcodeproj in Xcode and Run on My Mac.
...
For a self-contained, no-hardware validation of the receive pipeline:
    ...
    /tmp/e2e /tmp/remotecrab-e2e 30
EOF

echo ""
echo "═══════════════════════════════════════════════════════"
echo "  ✓ Simulator launched RemoteCrabCapture successfully"
echo "═══════════════════════════════════════════════════════"
echo ""
