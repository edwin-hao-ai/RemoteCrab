...
/// (green = streaming, orange = connecting, red = error). Tinted to
/// follow the system template so it adapts to light / dark menu bar
/// automatically.
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            let monitor = Path(roundedRect: CGRect(x: 1.2, y: 6, width: 17.6, height: 11.5),
                               cornerRadius: 3.2)
            ctx.stroke(monitor, with: .color(.white), lineWidth: 1.25)
            ...
            for (radius, opacity) in [(2.9, 0.9), (4.3, 0.45)] as [(CGFloat, Double)] {
                ...
                arc.addArc(center: CGPoint(x: 10, y: 2.3),
                           ...
                           clockwise: false)
                ctx.stroke(arc, with: .color(.white.opacity(opacity)), lineWidth: 0.9)
            }
