...
final class AppDelegate: NSObject, NSApplicationDelegate {
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if env["REMOTECRAB_SYSEX_REPAIR"] == "1" {
            SystemExtensionManager.shared.repair()
        } else if env["REMOTECRAB_SYSEX_ACTIVATE"] == "1" {
...
@main
struct RemoteCrabReceiverApp: App {
    ...
    ///   • `RemoteCrabAUInstanceProvider` (which the system extension
    ///     uses to vend the same instance into its own AUv3 instance)
    @State private var audioUnit: RemoteCrabAudioUnit? = {
        let unit = RemoteCrabAudioUnit()
        RemoteCrabAUInstanceProvider.makeInstance = unit
        return unit
    }()
...
/// Minimal "RemoteCrab is running" view shown in the root window after
