...
/// (`AudioReceiver` in RemoteCrabReceiver target); the consumer is the
/// audio render thread in the RemoteCrab virtual microphone.
public final class RemoteCrabRingBuffer: @unchecked Sendable {
...
/// RemoteCrab's virtual microphone — wraps the ring buffer with a small
/// public API the host process can use to enqueue Float32 PCM samples
/// from each decoded iPhone-mic `AudioPacket`.
///
/// In V0.2 we use this directly via `RemoteCrabAUInstanceProvider` for
/// the in-process path (simulator + dev). When the
/// `RemoteCrabAudioExtension.appex` is properly code-signed + installed,
...
public final class RemoteCrabAudioUnit: @unchecked Sendable {
    ...
    private let ringBuffer: RemoteCrabRingBuffer

    public init() {
        // 1 second of 48 kHz mono Float32 = 192 KB. Smooths minor
        // WiFi jitter between the host's enqueue and the render call.
        self.ringBuffer = RemoteCrabRingBuffer(capacityBytes: 48_000 * MemoryLayout<Float>.size)
    }
