...
│   ├── generate-ios-app-icons.py  # PIL: 1024x1024 → all iOS sizes
...
| Menu bar icon | `MenuBarIcon.swift` | Custom Canvas-drawn iPhone + WiFi waves |
...
- Custom App Icon (no text — minimalist iPhone + WiFi waves)
...
icon is generated by `scripts/generate-ios-app-icons.py` using PIL,
because we want pixel-perfect control over the output and `ImageRenderer`
